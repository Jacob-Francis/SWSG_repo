...
class SWSGSimulation:
    ...
    def height_func(
        self, x, f=1.0, g=0.1, a=0.1, c=0.5, d=1.0, profile_type="jet"
    ):
        if profile_type == "jet":
    ...
    def compute_4Ddense_samples(
        ...
        # Create 4D mesh:
        return self.mesh4D(G, Y), h_density / h_density.sum()
    ...
    def compute_W2_errors(
        ...
        _torch_numpy_process = lambda x: x.clone().detach().to(dtype=torch.float64, device=self.device) if isinstance(x, torch.Tensor) else torch.tensor(x, dtype=torch.float64, device=self.device)
        ...
        if which == 1:
            for key, target in {
                ...
                diff = periodic_g_x_vel(G, target, 1, L=1.0, periodic=False) - self.u_g(target)
