...
parser.add_argument("method", type=str, help="metho")
...
for j, dt in enumerate(dts):
    for k, epsilon in enumerate(epsilons):
        with open(f'data_store/output_{method}_{dt}_{epsilon}.pkl', 'rb') as f:
            data = pickle.load(f)
        ...
        for t in range(times_steps):
            if t == 0:
                ...
                output = loss(h_dense, X_dense, mu, tf(data[2][:, :, 0]))
            ...
            with open(f'data_store/x_star_lost_list_{method}_{dt}_{epsilon}_.pkl', 'wb') as f:
