...
#         G_xy = output[0][:, :, -1].view(-1, 2)
    ...
    f = open(f'data_store/simstudy_shorter_{method}_{dt}_{epsilon}_strength_{strength}.pkl', 'wb')
...
with open(f'data_store/simstudy_shorter_{method}_{E}_{E}_strength_{strength}.pkl', 'rb') as f:
...
uot_class.parameters(epsilon=dt)
...
for i in range(8):
    ...
    with open('shorter_fixed_eps_data.pkl', 'wb') as f:
        pickle.dump(s1, f)
...
# plt.savefig('shorter_fixed_eps_data.pdf')
...
for i in range(8):
    ...
    print('TIMING: ', tic-toc, output)
    s1.append(output.item())
    with open('shorter_height_data.pkl', 'wb') as f:
        pickle.dump(s1, f)
#  Look at solutions

# plt.loglog([np.sqrt(1/n) for n in [25**2, 35**2, 50**2, 71**2,100**2, 141**2]], [np.sqrt(t) if t > 1e-14 else 0 for t in s], '.:')
# plt.set_aspect('equal',plt.savefig('shorter_height_data.pdf')
#  adjustable='box')
# plt.xlabel(r'$\varepsilon^2$')
