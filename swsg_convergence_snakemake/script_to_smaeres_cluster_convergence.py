...
for epsilon in epsilons:
    ...
    # temp save
    with open(f"data_store/four_pertjet_sameres_conv_00156.pkl", "wb") as f:
    ...
    uotclass1.densities(X, X, approx_height, approx_height)
    ...
    save_dict[epsilon]['dual_cost22'] = sum(dual_cost22).item()
...
error_list = []
for epsilon in epsilons:
    error_list.append(save_dict[epsilon]['dual_cost'] - 0.5 * save_dict[epsilon]['dual_cost11'] - 0.5* save_dict[epsilon]['dual_cost22'])
...
plt.savefig(f"data_store/four_pertjet_sameres_conv_00156.png", dpi=300)